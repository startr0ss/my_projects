...
for i = 1 : c
    ...
    for j = 1 : k
        r = r + N(j);
        tmData1 = unifrnd(centers(j, i) - sqrt(6 * SD(j, i)), centers(j, i) + sqrt(6 * SD(j, i)), N(j), 1);
        tmData2 = unifrnd(centers(j, i) - sqrt(6 * SD(j, i)), centers(j, i) + sqrt(6 * SD(j, i)), N(j), 1);
        allData(l : r, i) = 0.5 * (tmData1 + tmData2);
        for e = l : r
...
% save dataWithoutErrorsTriangular.mat allData;